...
function calcularPromedio(listaDeNumeros) {
    ...
    return promedio = suma / listaDeNumeros.length
}
...
function triplicarLista(listaDeNumeros) {
    //
    return listaDeNumeros.map(numeros => numeros * 3);
}
...
function encontrarPalabraMasCorta(listaDePalabras) {
    ...
    let palabraActual = listaDePalabras[i];
    if (palabraActual.length < palabraMasCorta.length) {
...
function contarAprobados(listaDeNotas) {
    //
    const notasAprobadas = listaDeNotas.filter(nota => nota >= 7);
...
function enumerarLista(listaDePalabras) {
    //
    const ultimoElemento = listaDePalabras.pop(); // .pop() quita el último elemento y lo devuelve
    const elResto = listaDePalabras.join(", ");
    let oracion = `${elResto} y ${ultimoElemento}.`;
